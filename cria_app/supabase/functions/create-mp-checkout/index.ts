...
serve(async (req) => {
  ...
  try {
    ...
    if (!MP_ACCESS_TOKEN) {
      throw new Error(
        "Mercado Pago Access Token not configured (Secret MP_ACCESS_TOKEN missing)",
      );
    }
    ...
    const preferenceData = {
      ...
      notification_url:
        "https://drkuxfafxoruuvszowld.supabase.co/functions/v1/mp-webhook",
      payment_methods: {
    ...
    if (!mpResponse.ok) {
      ...
      throw new Error(
        `MP Error ${mpResponse.status}: Falha ao gerar link do Mercado Pago`,
      );
    }
